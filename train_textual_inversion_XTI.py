...
def train(args):
    ...
    print("running training / 开始训练")
    print(f"  num train images * repeats / 学习图像的数量×重复次数: {train_dataset_group.num_train_images}")
    print(f"  num reg images / 正则化图像的数量: {train_dataset_group.num_reg_images}")
    print(f"  num batches per epoch / 每个epoch的批数: {len(train_dataloader)}")
    print(f"  num epochs / epoch数: {num_train_epochs}")
    print(f"  batch size per device / バッチサイズ: {args.train_batch_size}")
    print(f"  total train batch size (with parallel & distributed & accumulation) / 总批量大小(包括并行学习、梯度求和): {total_batch_size}")
    print(f"  gradient ccumulation steps / 合计梯度的步骤数 = {args.gradient_accumulation_steps}")
    print(f"  total optimization steps / 学习步数: {args.max_train_steps}")
