...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        if args.max_train_epochs is not None:
            ...
            accelerator.print(
                f"override steps. steps for {args.max_train_epochs} epochs is / 到达指定时代的步数: {args.max_train_steps}"
            )
        ...
        accelerator.print(f"  num train images * repeats / 学习图像的数量×重复次数: {train_dataset_group.num_train_images}")
        ...
        accelerator.print(
            f"  batch size per device / 批量大小: {', '.join([str(d.batch_size) for d in train_dataset_group.datasets])}"
        )
        # accelerator.print(f"  total train batch size (with parallel & distributed & accumulation) / 总批量大小(包括并行学习、梯度求和): {total_batch_size}")
